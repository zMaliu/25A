function [result,dur,posRelease,posBurst]=F_q4_3points(angle_fly,v_fly,pos0_fly,param)
    ...
    bestT2 = 0;
    for t = 1:1:13
        dir_fly = [cosd(angle_fly), sind(angle_fly), 0];
    ...
    [~, idx] = sort(all_distances);
    for i = 1:13
        sorted_idx = idx(i);
        points{i} = best_points(sorted_idx).point;
        [~, ~, ~, tRelease_temp{i}] = F_q3getdur(points{i}, v_fly, angle_fly,pos0_fly,param);
    end
    ...
    for i = 1:3
        ...
        points{i} = best_points(sorted_idx).point;
        [posBurst{i}, posRelease{i}, dur{i}, tRelease{i}] = F_q3getdur(points{i}, v_fly, angle_fly,pos0_fly,param);
    end
    dur_vals = [dur{:}];  
